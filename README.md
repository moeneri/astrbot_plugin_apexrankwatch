...
Apex Rank Watch 是一个面向 QQ 群使用的 AstrBot 插件，用来查询 Apex Legends 玩家段位、地图轮换、当前赛季结束时间和剩余时间、猎杀线，并支持群内持续监控与记录排位分变化。
...
- 信息指令组：`/map`、`/全天地图`、`/匹配地图`、`/apexseason`、`/apexpredator [平台]`、`/猎杀`
  查询排位地图、未来 24 小时地图排期、三人赛地图、赛季结束时间和剩余时间、猎杀线。
...
- 赛季结束时间和剩余时间优先来自公开倒计时页面；页面的新赛季数据延迟更新时，会回退 EA 官方当前赛季页。赛季开始、结束和新赛季切换边界统一按北京时间凌晨 1 点展示和计算。
...
- 赛季倒计时和部分赛季信息优先参考 `apexseasons.online`；新赛季数据延迟更新时回退 EA 官方当前赛季页。
